...
cookies_path = os.path.join(res_dir, "cookies.json")
stealth_js_path = os.path.join(res_dir, "stealth.min.js")
qr_code_path = os.path.join(res_dir, "QRcode.jpg")
...
async def init_page(p: Playwright) -> tuple[Page, BrowserContext]:
    driver = "msedge" if config.driver == "edge" else "chrome"
    ...
    # 如果有自定义浏览器路径，不使用channel
    if config.exe_path:
        launch_args['executable_path'] = config.exe_path
    else:
...
async def auto_login(context: BrowserContext, page: Page, modules=None):
    async def request_handler(request):
        if "https://www.zhihuishu.com" in request.url:
            ...
            logger.info(f"已保存登录凭证到: {cookies_path},下次可免密登录.")
            # 停止监听
...
if __name__ == "__main__":
    ...
    try:
        logger.info("程序启动中...")
        import os
        config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "configs.ini")
